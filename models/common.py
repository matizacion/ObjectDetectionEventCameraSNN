...
class Snn_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        weight = self.weight
        h = (input.size()[-2]-self.kernel_size[0]+2*self.padding[0])//self.stride[0]+1
        w = (input.size()[-1]-self.kernel_size[0]+2*self.padding[0])//self.stride[0]+1
        c1 = torch.zeros(time_window, input.size()[0], self.out_channels, h, w, device=input.device)
        # c1 = torch.zeros(input.size()[0], self.out_channels, h, w, device=input.device)
        # c1 = torch.zeros(input.size()[0], self.out_channels, h, w, device=input.device)
        # print(weight.size(),'=====weight====')
        # print(c1.size(),'=====c1====')
        # print(grayscale.size(),'=====grayscale====')
        print(input.size(),'=====input====')
        for i in range(time_window):
            c1[i] = F.conv2d(input, weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        # print(c1.sum(),'====c1.sum()====')
        # print(c1.dtype,'====c1.dtype====')
        # print(c1[-1].size(),'====c1[-1].dtype====')
        # return c1[-1]
        return c1
