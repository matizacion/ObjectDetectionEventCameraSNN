...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        #M.K
        # dont know what is savelist
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch])  # model, savelist

        # print(self.model)

        # self.model = nn.Sequential(
            # *[snn.Leaky(beta = 0.8, init_hidden = True) if isinstance(m, Conv) else m for m in self.model]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    #Conv.default_act = nn.Identity()  # redefine default activation, i.e. Conv.default_act = nn.SiLU()
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        #print(f'#1',f, n, m, args)
        m = eval(m) if isinstance(m, str) else m  # eval strings
        #print(f'#2',f, n, m, args)
        for j, a in enumerate(args):
            with contextlib.suppress(NameError):
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings
        #print(f'#3',f, n, m, args)
        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain
        #print(f'#4',f, n, m, args)
        if m in {
            Conv,
            SConv,  #M.K
            GhostConv,
        ...
        #print(f'#9',m_.i, m_.f, m_.type, m_.np)
        #M.K.
        # print(f,n,m,args, m_.i, m_.f, m_.type, m_.np)
